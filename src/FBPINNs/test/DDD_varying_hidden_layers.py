...
def plot_DDD_varying_hidden_layers():
    ...
    params_type = [1,  [0.5, 0.7, 0.3, 0.3, 0.6], [0.5, 0.3, 0.6, 0.7, 0.3]]
    ...
    for output_size in output_sizes:
        varying_layers_for_model =[]
        for hidden_layers in varying_layers:
            current_layer_config = [1] + hidden_layers + [output_size]
            varying_layers_for_model.append(current_layer_config)
        layer_configs.append(varying_layers_for_model)
    ...
    for (layers, problem, params, rootdir, name) in zip(layer_configs, problem_types, params_type, rootdirs_with_parent, cases):
        ...
        for layer in layers:
            ...
            network_kwargs_set.append((network_init_kwargs))
        ...
        for (problem_kwargs, tl) in problem_kwargs_set:
            for (decomposition_kwargs, tld) in decomposition_kwargs_set:
                if tl==tld:
                    for (network_kwargs) in network_kwargs_set:
                        run=f"FBPINNs_{tag}_{nsub}_nsub_{wo}_wo_{tl}_tl_{network_kwargs["layer_size"]}_layer_{name}"
                        runs.append(run)
                        h = len(network_kwargs["layer_size"]) -2
                        p = sum(network_kwargs["layer_size"][1:-1])
                        c = ModifiedConstants(
                            run=run,
                            domain=domain,
                            ...
                            network=network,
                            network_init_kwargs=network_kwargs,
        ...
        for run in runs:
            c_out, model = load_model(run, rootdir=rootdir+"/")
            layer = c_out.network_init_kwargs["layer_size"]
            ll = len(layer) - 2
            hu = layer[1]
            l_str = str(f"{ll}x{hu}")
            ...
            if column_index != -1:
                ...
                for i in range(0, u_exact.shape[1]):
                    models_ax[0, column_index].plot(x_batch, u_learned[:,i], '-.', label=f"{labels[i]}-{l_str}")
                    models_ax[1, column_index].plot(x_batch, u_test[:,i], ':', label=f"{labels[i]}-{l_str}") 
        ...
        for i in range(2):  # For each row
            for j in range(2):  # For each column
                ...
                models_ax[i][j].legend(loc='upper center', bbox_to_anchor=(0.5, -0.15), shadow=True, ncol=4 if problem.__name__=="SaturatedGrowthModel" else 5)
        models_fig.tight_layout()
        file_path = f"{parentdir}/Models_plot({name}).png"
        models_fig.savefig(file_path)

        # plot text, heatmap and loss
        ...
        # only one legend for 3 individual loss plot.
        colors1 = cm.Blues(np.linspace(0.2,0.95,len(layers)))
        colors2 = cm.Greens(np.linspace(0.2,0.95,len(layers)))
        colors3 = cm.Reds(np.linspace(0.2,0.95,len(layers)))
        # colors4 = cm.Greys(np.linspace(0.2,0.95,6))
        colors = np.vstack((colors1, colors2, colors3))
        np.random.seed(42)
        np.random.shuffle(colors)
        # colors = plt.cm.viridis(np.linspace(0, 1, len(layers)))
        color_map = {str(layer): color for layer, color in zip(layers, colors)}

        # Create Dataset for plotting
        df = pd.DataFrame(columns=['Time Limit', 'Layer', 'MSE Learned', 'MSE Test'])
        for run in runs:
            c_out, model = load_model(run, rootdir=rootdir+"/")

            layer = str(c_out.network_init_kwargs["layer_size"])
            ...
            ax.plot(i, l1n, label=f"l-{layer}", color=color)
            ax.set_yscale('log')
        ...
        pivot_learned = df.pivot(index="Layer", columns="Time Limit", values="MSE Learned")
        pivot_test = df.pivot(index="Layer", columns="Time Limit", values="MSE Test")
        ...
        plt.suptitle('MSE Value by Time Limit and Varying Layer Size', fontsize=14, verticalalignment='top')#, y=0.95)
...
if __name__=="__main__":
    plot_DDD_varying_hidden_layers()
