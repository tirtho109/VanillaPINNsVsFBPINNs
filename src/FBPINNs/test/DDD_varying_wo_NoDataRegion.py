...
def plot_DDD_varying_wo_NoDataRegion():
    ...
    parentdir = "DDD_wo_NoDataRegion"
    rootdirs = ["DDD_wo_NoDataRegion_sg", "DDD_wo_NoDataRegion_coexistence", "DDD_wo_NoDataRegion_survival"]
    ...
    for (layer, problem, params, rootdir, name) in zip(layers, problem_types, params_type, rootdirs_with_parent, cases):
        ...
        for wi in wis:
            for tl in time_limits:
                ...
                decomposition_kwargs_set.append((decomposition_init_kwargs, wi, tl))
        ...
        for (problem_kwargs, tl) in problem_kwargs_set:
            for (decomposition_kwargs, wi, tld) in decomposition_kwargs_set:
                if tl==tld:
                    run = f"FBPINN_{tag}_{nsub}_subdomains_{wo}_wo_{wi}_wi_{tl}_tl"
                    runs.append(run)
                    c = ModifiedConstants(
                        run=run,
                        domain=domain,
        ...
        gs_lossplot = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=gs[2], wspace=0.3) 
        ...
        df = pd.DataFrame(columns=['Time Limit', 'Window NoDataRegion', 'MSE Learned', 'MSE Test'])
        ...
        for run in runs:
            ...
            new_row = pd.DataFrame({'Time Limit': [tl_key], 'Window NoDataRegion': [wi],
        ...
        pivot_learned = df.pivot(index="Window NoDataRegion", columns="Time Limit", values="MSE Learned")
        pivot_test = df.pivot(index="Window NoDataRegion", columns="Time Limit", values="MSE Test")
...
if __name__=="__main__":
    plot_DDD_varying_wo_NoDataRegion()
