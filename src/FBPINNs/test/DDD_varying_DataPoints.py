...
def plot_DDD_varying_DataPoints():
    ...
    parentdir = "DDD_DataPoints"
    rootdirs = ["DDD_DataPoints_sg", "DDD_DataPoints_coexistence", "DDD_DataPoints_survival"]
    rootdirs_with_parent = [os.path.join(parentdir, rd) for rd in rootdirs]

    # collector dataframes
    collcted_df = pd.DataFrame(columns=['Time Limit', 'Data Points', 'MSE Learned', 'MSE Test', 'Model Type', 'Learned Parameters'])
    ...
    for (layer, problem, params, rootdir, name) in zip(layers, problem_types, params_type, rootdirs_with_parent, cases):
        ...
        for (problem_kwargs, tl) in problem_kwargs_set:
            for (decomposition_kwargs, tld) in decomposition_kwargs_set:
                if tl==tld:
                    run = f"FBPINN_{tag}_{name}_{problem_kwargs["numx"]}_numx_nsub_{problem_kwargs["noise_level"]}_noise_{tl}_tl"
                    runs.append(run)
                    c = ModifiedConstants(
                        run=run,
                        domain=domain,
        ...
        for run in runs:
            ...
            if column_index != -1:
                ...
                for i in range(0, u_exact.shape[1]):
                    models_ax[0, column_index].plot(x_batch, u_learned[:,i], '-.', label=f"{labels[i]}-{dp}")
                    models_ax[1, column_index].plot(x_batch, u_test[:,i], ':', label=f"{labels[i]}-{dp}") 
        ...
        df = pd.DataFrame(columns=['Time Limit', 'Data Points', 'MSE Learned', 'MSE Test'])
        for run in runs:
            c_out, model = load_model(run, rootdir=rootdir+"/")

            parts = c_out.run.split("_")
            data_index = 3
            data_points = parts[data_index]
            ...
            ax.plot(i, l1n, label=f"Data-{data_points}")
            ax.set_yscale('log')
            ...
            new_row = pd.DataFrame({'Time Limit': [tl_key], 'Data Points': [data_points],
            ...
            new_collcected_row = pd.DataFrame({'Time Limit': [tl_key], 'Data Points': [data_points],
                                'MSE Learned': [mse_learned], 'MSE Test': [mse_test],
        ...
        df['Data Points'] = df['Data Points'].astype(int)
        df = df.sort_values('Data Points')

        pivot_learned = df.pivot(index="Data Points", columns="Time Limit", values="MSE Learned")
        pivot_test = df.pivot(index="Data Points", columns="Time Limit", values="MSE Test")
        ...
        plt.suptitle('MSE Value by Time Limit and Data Points', fontsize=14, verticalalignment='top')#, y=0.95)
        plt.subplots_adjust(hspace=0.2, top=0.88)
        plt.tight_layout()
        file_path = f"{parentdir}/MSE_varying_DataPoints({name}).png"
        plt.savefig(file_path)

        # export collected data
        collcted_df.to_csv(f"{parentdir}/FBPINN_collected_info_varying_DataPOints.csv", index=False)

        print("DONE")

if __name__=="__main__":
    plot_DDD_varying_DataPoints()