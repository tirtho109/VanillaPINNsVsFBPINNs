...
def train_FBPINN_models():
    ...
    sampler='grid'
    nsub = 2
    delta_weights = 1.0 # ll direction span
    ...
    # create fig for sciann vs fbpinns
    ...
    for (layer, problem, params, rootdir, name, fig_tag) in zip(layers, problem_types, params_type, root_dirs_with_parent, cases, fig_tags):
            ...
            for (decomposition_kwargs, tld) in decomposition_kwargs_set:
                if tl==tld:
                    ...
                    if args.loss_landscape[0]:
                        ...
                        _loss_lanscape(FBPINNrun, run, rootdir+"/", 20, 2, c.summary_out_dir, delta_weights=delta_weights)
                        # plot loss landscape
                    ...
                    # # plot sciann vs fbpinns
                    # if name=='coexistence':
                    #     # import sciann outputs
                    #     sciann_path = os.path.join(parent_dir, "coexistence_sciann", f"SciANN_CM_coexistence_actf_tanh_l_5x5x5_nD_100_bs_300_e_50000_lr_0.001_tl_{tl[0]}-{tl[1]}_nl_0.05_nC_200_nT_500_")
                    #     sciann_pred_solution = np.loadtxt(sciann_path + '/' + "res_t_u_v_pred.csv", delimiter=',', skiprows=1)
                    #     print(sciann_pred_solution.shape)

                    #     # export u_test and x_batch using  np.savetxt()
                    #     np.savetxt(f"{c.summary_out_dir}/res_u_v_pred.csv", u_test, delimiter=',', header='u,v', comments='')
                    #     np.savetxt(f"{c.summary_out_dir}/res_x_batch.csv", x_batch, delimiter=',', header='t', comments='')
                    #     # save u_exact
                    #     np.savetxt(f"{c.summary_out_dir}/res_u_exact.csv", u_exact, delimiter=',', header='u,v', comments='')

                    #     for i in range(0, u_exact.shape[1]):
                    #         ax_sciann_vs_fbpinns[time_limits.index(tl)].plot(x_batch, u_test[:,i:i+1], label=f"{labels[i]} FBPINN", linestyle=(0, (3, 1, 1, 1, 1, 1)), color=colors[i])
                    ...
                    #     #ax_sciann_vs_fbpinns[time_limits.index(tl)].set_xlabel('Time domain', fontsize=24)
        ...
        if name=='coexistence':
            for i in range(0, u_exact.shape[1]):
                for tl in time_limits:
                    ax_sciann_vs_fbpinns[time_limits.index(tl)].plot(x_batch, u_exact[:,i:i+1], label=f"{labels[i]} True", linestyle='-', color=colors[i])
...
#     desired_order = ['u FBPINN', 'u PINN', 'u True', 'v FBPINN', 'v PINN', 'v True']
#     ordered_handles = [label_handle_dict[label] for label in desired_order]
#     ordered_labels = desired_order

#     fig_sciann_vs_fbpinns.legend(ordered_handles, ordered_labels, loc='lower center', bbox_to_anchor=(0.53, -0.05), ncol=6, fontsize=20)
#     fig_sciann_vs_fbpinns.tight_layout(rect=[0, 0.05, 1, 1])
#     fig_sciann_vs_fbpinns.savefig(f"{parent_dir}/sciann_vs_fbpinns_model_comparison(coexistence).png", bbox_inches='tight')
    # tikzplotlib.save(f"{parent_dir}/sciann_vs_fbpinns_model_comparison(coexistence).tex", figure=fig_sciann_vs_fbpinns)
    # matplotlib2tikz.save(f"{parent_dir}/sciann_vs_fbpinns_model_comparison(coexistence).tex", figure=fig_sciann_vs_fbpinns)
