...
def submit(runs):
    ...
    if args.plot:
        df = pd.DataFrame(columns=["Time Limit", "Model Type", "MSE Learned", "MSE Test", "Level", "Hidden Layers and Units"])
        for run_config, run_type, rootdir in runs:
            if run_type == "FBPINN":
                # 0. load model
...
parentdir = "SLvsML/SurvMultilevel"
# Competition Model
...
tag = "Coex"
parentdir = "SLvsML/CoexMultilevel"
