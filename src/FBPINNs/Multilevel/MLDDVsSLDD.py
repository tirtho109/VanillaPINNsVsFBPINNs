...
def submit(runs):
    ...
    if args.plot:
        ...
        parameters_df = pd.DataFrame(columns=['tag', 'model_type', 'time_limit', 'true_params', "Level", 'learned_params'])
        for run_config, run_type, rootdir in runs:
            if run_type == "FBPINN":
                # 0. load model
                ...
                parameters_csv_pattern = os.path.join(run_config.summary_out_dir, "parameters.csv")
                ...
                new_row_df = pd.DataFrame([{
                    'tag': 'FBPINN',
                    'model_type': run_config.run.split("_")[1],
                    'time_limit': str(run_config.problem_init_kwargs["time_limit"]), 
        ...
        file_path = ("MLDDVsSLDD_wo_param/summary.csv")
        df.to_csv(file_path, index=False)
        # export the parameters_df to csv   
        file_path = ("MLDDVsSLDD_wo_param/combined_parameters.csv")
...
tag = "SG"
parentdir = "MLDDVsSLDD_wo_param/SGMultilevel"
...
parentdir = "MLDDVsSLDD_wo_param/SurvMultilevel"
# Competition Model
...
tag = "Coex"
parentdir = "MLDDVsSLDD_wo_param/CoexMultilevel"
