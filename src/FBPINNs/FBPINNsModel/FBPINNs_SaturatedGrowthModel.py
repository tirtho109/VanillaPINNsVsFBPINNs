...
def get_parser():
    ...
    parser.add_argument('-nx','--numx', help='Number of data points (default 50)', type=int, default=100)
    parser.add_argument('-tl', '--time_limit', help='Time window for the training data (default [0, 24])', type=int, nargs=2, default=[0, 24])

    parser.add_argument('-nsub','--num_subdomain', help='Number of sub-domains (default 2)', type=int, default=3)
    ...
    parser.add_argument('-pt','--pinn_trainer', help='Whether to train PINN trainer (default False)', type=bool, nargs=1, default=[False])
...
def train_sg_model():
    ...
    run = f"FBPINN_{tag}_SG_{network.__name__}_{args.num_subdomain}-ns_{args.window_overlap}-l_{h}-l_{p}-h_{n[0]}-nC_"
    ...
    # check number of collocaiton points in the overlaps.
    subdomain_xs, subdomain_ws = get_subdomain_xsws(args.time_limit, args.tbegin, args.tend, 
                                                    args.num_subdomain, args.window_overlap, args.window_noDataRegion)
    collocation_points = np.linspace(domain_init_kwargs['xmin'][0], domain_init_kwargs['xmax'][0], args.num_collocation)
    print(subdomain_xs, subdomain_ws)
